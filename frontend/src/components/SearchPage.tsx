          ...
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-3 my-3">
              <div className="flex flex-wrap gap-2 mb-4 md:mb-8">
                <p className="font-medium">(2) Search result found</p>
                {["1", "2", "3"].map((selectedFilter: string, idx: number) => {
                  return (
                    <div
                      key={idx}
            ...
            <div className="grid md:grid-cols-4 gap-4 ">
              {[1, 2, 3].map((items: number, idx: number) => (
                <Card className="bg-white dark:bg-gray-800 shadow-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                  <div className="relative">
                    ...
                    <div className="absolute top-2 bg-white dark:bg-green-700 bg-opacity-75 rounded-lg py-1 px-3">
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex mt-4 gap-2 flex-wrap">
                      {["1254651", "2546515", "3846121"].map(
                        (cuisine: string, idx: number) => (
                          <Badge
                            key={idx}
                            className="py-1 px-2 font-medium rounded-full shadow-sm"
                          >
                            {cuisine}
                          </Badge>
                        )
                  ...
                  <CardFooter className="p-4 border-t dark:border-t-gray-700 border-t-gray-100 text-white flex justify-end">
                    <Link to={`/restaurant/${123}`}>
                      <Button className="text-sm py-0 rounded-2xl">View Menus</Button>
                    </Link>
                  </CardFooter>
                </Card>
              ))}
